...
export class ImportFileComponent implements OnInit {
  ...
  onSubmit() {
   ...
   this.new_descmain=this.uploadForm.get('descmain').value;
    ...
    && !this.isEmpty(this.new_type)
    && !this.isEmpty(this.new_format)
    && !this.isEmpty(this.new_descmain)
    )
  {
    if(this.new_format.length>5)
    {
      this.toastr.error('', 'Format length should be less than 5!');
    }
    
    else{
    const formData = new FormData();
    formData.append('audit_session_id',localStorage.getItem('editSessionId'));
    formData.append('mainFile', this.uploadForm.get('mainFile').value);
    formData.append('file_name', this.uploadForm.get('file_name').value);
    formData.append('file_type', this.uploadForm.get('type').value);
    formData.append('format', this.uploadForm.get('format').value);
    formData.append('descmain', this.uploadForm.get('descmain').value);
    ...
    this.rest.importFile(formData).subscribe((data: any) => {
      // debugger;
